...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const ollamaUrlInput = document.getElementById('ollamaUrl') as HTMLInputElement | null;
  const ollamaModelInput = document.getElementById('ollamaModel') as HTMLInputElement | null;
  ...
  chrome.storage.local.get(
    ['provider', 'openaiApiKey', 'ollamaUrl', 'ollamaModel'],
    (result: Settings) => {
      if (result.provider) {
  ...
  saveBtn?.addEventListener('click', () => {
    const settings: Settings = { provider: currentProvider };
